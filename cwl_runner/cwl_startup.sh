...
WORKFLOW_FILE=$(curl "${METADATA_URL}/attributes/workflow-file" -H "${METADATA_HEADERS}")
SETTINGS_FILE=$(curl "${METADATA_URL}/attributes/settings-file" -H "${METADATA_HEADERS}")
INPUT=$(curl "${METADATA_URL}/attributes/input" -H "${METADATA_HEADERS}")
INPUT_RECURSIVE=$(curl "${METADATA_URL}/attributes/input-recursive" -H "${METADATA_HEADERS}")
DISK_NAME=google-$(curl "${METADATA_URL}/disks/1/device-name" -H "${METADATA_HEADERS}")
RUNNER=$(curl "${METADATA_URL}/attributes/runner" -H "${METADATA_HEADERS}")
...
INPUT_FOLDER="${MOUNT_POINT}/input"
OUTPUT_FOLDER="${MOUNT_POINT}/output"
TMP_FOLDER="${MOUNT_POINT}/tmp"
...
WORKFLOW_LOCAL="${INPUT_FOLDER}/${WORKFLOW_FILE//:\//}"
...
SETTINGS_LOCAL="${INPUT_FOLDER}/${SETTINGS_FILE//:\//}"
...
if [[ ${KEEP_ALIVE} = "true" ]]; then
  ...
  echo "VM will shutdown automatically"
fi
